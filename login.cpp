...
//    QSqlQuery query(dataBase);
    ...
    query.clear();
    query.exec(sql);
    qDebug()<<"query.exec(sql);"<<endl;
    if(!query.seek(0)){
        QMessageBox::warning(this,tr("info"),tr("login fail"), QMessageBox::Ok);
        query.finish();
        query.clear();
        getDB().close();
        return;
    }else{
//        userId = query.value(0).toString(); // 保存用户id
        userName = query.value(0).toString(); // 保存用户名字
        qDebug()<<"login "<< userId<<userName<<endl;
        query.finish();
        query.clear();
        getDB().close();
        QMessageBox::warning(this,tr("info"),tr("login success"), QMessageBox::Ok);
        saveSetting();
        accept();
    }
    return;
